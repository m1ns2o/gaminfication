...
- `GameRoom` Durable Object는 진행 중인 방의 턴, 말 위치, 주사위, 퀴즈 제한시간·채점, 카드 효과와 연결 상태를 담당합니다.
...
- `app/api/v1/rooms/:id/results`: 종료된 수업의 순위·점수·정답 수 조회
- `worker/index.ts`: `/ws/rooms/:roomId` Upgrade 요청 라우팅

교육용 규칙은 방 생성, 참가, 시작, 서버 주사위, 교사가 구성한 24개 칸, 퀴즈 출제·제한시간·서버 채점, 점수와 카드 효과를 구현합니다. 순환형은 목표 점수·라운드 종료를, 직선형은 마지막 칸 완주를 지원합니다. 종료 결과는 D1에 저장되고 같은 탭을 새로고침해도 방 세션을 복원합니다. 땅 구매와 통행료는 의도적으로 포함하지 않습니다.
