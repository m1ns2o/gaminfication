...
교육용 규칙은 방 생성, 개인전·팀전 참가, 시작, 서버 주사위, 교사가 구성한 24개 칸, 현재 차례/전원 동시 퀴즈, 제한시간·서버 채점, 팀/개인 점수와 카드 효과를 구현합니다. 보드는 순환형·지그재그·일직선·나선형 네 종류이며, 순환형은 목표 점수·라운드 종료를, 나머지는 마지막 칸 완주를 지원합니다. 주사위 3D 회전, 칸별 말 이동과 도착 피드백은 서버가 확정한 상태를 순서대로 표현합니다. 종료 결과와 문항별 응답은 D1에 저장되고 같은 탭을 새로고침해도 방 세션을 복원합니다. 땅 구매와 통행료는 의도적으로 포함하지 않습니다.
