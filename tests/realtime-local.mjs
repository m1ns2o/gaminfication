...
const baseUrl = new URL(process.env.CLASSLOOP_BASE_URL ?? "http://localhost:3000");
...
const sql = [
  `INSERT INTO teacher_accounts (id, email, display_name, created_at, updated_at) VALUES ('${teacherId}', 'realtime-test@classloop.local', '실시간 테스트 교사', '${now.toISOString()}', '${now.toISOString()}') ON CONFLICT(id) DO UPDATE SET updated_at=excluded.updated_at`,
...
process.env.CLASSLOOP_TEST_SESSION_TOKEN = sessionToken;
